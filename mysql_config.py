st.markdown("""
    <div style="text-align: center; color: #666; padding: 20px;">
        <p>⚡ EV Charging Analytics Dashboard | MySQL Backend | Real-time Updates</p>
        <p>Data updates automatically every 30 seconds | Last refresh: {} CEST</p>
        <p>Built with Streamlit | Deploy on Streamlit Cloud for 24/7 monitoring</p>
    </div>
...
def handle_deployment_errors():
    ...
    try:
        ...
        st.stop()"""
EV Charging Stations Analytics Dashboard - Complete MySQL Version
Combines comprehensive analytics with MySQL connectivity for online deployment
Auto-refreshes from MySQL database with full feature set
"""
...
# Auto-refresh setup - real-time updates every 30 seconds
...
# Data loading functions with MySQL connectivity only
...
# Helper function to get current CEST time
def get_cest_time():
...
def check_auto_refresh():
    """Check if it's time to auto-refresh (every 30 seconds)"""
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Select Dashboard",
            ["📊 Overview", "🗺️ Station Map", "📈 Utilization Analytics", 
        ...
        auto_refresh = st.checkbox("Real-time refresh (30s)", value=st.session_state.auto_refresh_enabled)
        ...
        if not utilization_df.empty and 'timestamp' in utilization_df.columns:
            ...
            if data_age < 5:
                st.success(f"📡 Data current ({data_age:.1f}m)")
            elif data_age < 30:
...
def show_overview(stations_df, utilization_df, hourly_df, sessions_df):
    ...
    if not sessions_df.empty or not utilization_df.empty:
        ...
        # Add occupancy data
        if not utilization_df.empty and 'hourly_timestamp' in utilization_df.columns:
...
def show_utilization_analytics(utilization_df, hourly_df, sessions_df):
    """Show comprehensive utilization analytics"""
    ...
    # Filter datasets
    filtered_util = utilization_df[utilization_df['timestamp'] >= time_filter] if not utilization_df.empty else utilization_df
    ...
    with tab3:
        ...
        if not filtered_sessions.empty:
            ...
            if 'connector_type' in filtered_sessions.columns:
                ...
                type_stats.columns = ['total_energy', 'avg_energy', 'total_revenue', 'avg_revenue', 'avg_duration', 'session_count']
    ...
    with tab4:
        ...
        if not filtered_sessions.empty:
            ...
            # Extract station from connector_id
            top_connectors['station'] = top_connectors['connector_id'].str.extract(r'(STATION_\d+)')

            st.dataframe(
                top_connectors[['connector_id', 'station', 'total_revenue', 'total_energy', 'session_count']].rename(columns={
...
def add_footer():
    ...
    <div style="text-align: center; color: #666; padding: 20px;">
        ...
        <p>Data updates automatically every 60 seconds | Last refresh: {}</p>
        <p>Built with Streamlit | Deploy on Streamlit Cloud for 24/7 monitoring</p>
    </div>
...
def handle_deployment_errors():
    ...
    try:
        # Test database connectivity
        if MYSQL_AVAILABLE:
            test_df = load_from_mysql('charging_stations')
            if test_df.empty:
                st.warning("Database connected but no data found. Using sample data for demonstration.")
        else:
            st.info("Running in demo mode with sample data. Connect to MySQL database for live data.")
    except Exception as e:
        st.error(f"Database connection issue: {e}")
        st.info("Falling back to sample data for demonstration purposes.")
